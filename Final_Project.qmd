...
df_LASSO_cv <- cv.glmnet(x = df_x, # automatically includes a column of ones for the intercept FYI
                          ...
                          type.measure = "auc", 
                          alpha = .5, 
                          family = 'binomial') # 0 is code for "ridge regression"
...
```{r}
df.glm = glm(diagnosis ~ radius_mean + texture_mean + 
              `concave points_mean` + smoothness_mean + symmetry_mean, 
            data = df, family = binomial(link = 'logit'))
df.lm <- lm(ifelse(diagnosis == 'M', 1, 0) ~ radius_mean + texture_mean + `concave points_mean` + smoothness_mean + symmetry_mean, data = df)
